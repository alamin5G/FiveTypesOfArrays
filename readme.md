...
#### 4. Fixed Heap-Dynamic (with ArrayList comparison)
...
6. Fixed Heap-Dynamic (List Append): 229.70 ms
...
```java // Static int[] staticArray = new int[1000000];
...
Run the Java program in IntelliJ IDEA and follow the prompts to input array sizes when requested. The program will display performance metrics for each array type.

For the Python implementation, run the script in PyCharm using the equivalent Python code structure to compare performance characteristics.